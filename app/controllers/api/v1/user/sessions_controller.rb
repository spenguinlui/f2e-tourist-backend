  ...
  def google_oauth2
    ...
    if not code.blank?
      begin
        ...
        render :json => { message: "成功連結 google 使用者", token: @tokens, status: 200 }
      rescue Exception => e
        logger.error "----- google 第三方登入發生錯誤！！！ -> #{e}"
  ...
  def facebook
    ...
    if not code.blank?
      begin
        ...
        render :json => { message: "成功連結 facebook 使用者", token: @tokens, status: 200 }
      rescue Exception => e
        logger.error "----- facebook 第三方登入發生錯誤！！！ -> #{e}"
        render :json => { message: "發生不明錯誤", status: 500 }, :status => :bad_request
      end
    else
      render :json => { message: "無參數傳入", status: 400 }
